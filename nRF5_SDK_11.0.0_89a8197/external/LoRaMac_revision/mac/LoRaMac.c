...
APP_TIMER_DEF(MacStateCheckTimer);
static uint32_t MacStateCheckTimerTime;
...
static uint32_t TxDelayedTimerTime;
...
static uint32_t RxWindowTimer1Time;
APP_TIMER_DEF(RxWindowTimer2);
static uint32_t RxWindowTimer2Time;
...
static uint32_t AckTimeoutTimerTime;
...
{
	return (uint32_t)((uint64_t)(ticks*1000)/32768);
}

TimerTime_t TimerGetCurrentTime(void)
{
	TimerTime_t tmpTime;
	app_timer_cnt_get(&tmpTime);
	return tickToMs(tmpTime);
}

 TimerTime_t TimerGetElapsedTime( TimerTime_t eventInTime )
{
    ...
    { // roll over of the counter
        return( elapsedTime + ( tickToMs(MAX_RTC_COUNTER_VAL+1) - eventInTime ) );
    }
    else
    {
...
static void OnRadioTxDone( void )
{
    ...
    if( IsRxWindowsEnabled == true )
    {
		...
		RxWindowTimer1Time = RxWindow1Delay;
        //TimerStart( &RxWindowTimer1 );
		app_timer_start(RxWindowTimer1, APP_TIMER_TICKS(RxWindowTimer1Time, APP_TIMER_PRESCALER), NULL);
        if( LoRaMacDeviceClass != CLASS_C )
        {
            //TimerSetValue( &RxWindowTimer2, RxWindow2Delay );
			RxWindowTimer2Time = RxWindow2Delay;
            //TimerStart( &RxWindowTimer2 );
			app_timer_start(RxWindowTimer2, APP_TIMER_TICKS(RxWindowTimer2Time, APP_TIMER_PRESCALER), NULL);
        }
        if( ( LoRaMacDeviceClass == CLASS_C ) || ( NodeAckRequested == true ) )
        {
            //TimerSetValue( &AckTimeoutTimer, RxWindow2Delay + ACK_TIMEOUT +
                                             //randr( -ACK_TIMEOUT_RND, ACK_TIMEOUT_RND ) );
			AckTimeoutTimerTime =	RxWindow2Delay + ACK_TIMEOUT +
                                             randr( -ACK_TIMEOUT_RND, ACK_TIMEOUT_RND )	;	 
            //TimerStart( &AckTimeoutTimer );
			app_timer_start(AckTimeoutTimer, APP_TIMER_TICKS(AckTimeoutTimerTime, APP_TIMER_PRESCALER), NULL);
        }
    }
    else
    {
...
static void PrepareRxDoneAbort( void )
{
	...
	MacStateCheckTimerTime = 1;
    //TimerStart( &MacStateCheckTimer );
	app_timer_start(MacStateCheckTimer, APP_TIMER_TICKS(MacStateCheckTimerTime, APP_TIMER_PRESCALER), NULL);
}
...
static void OnMacStateCheckTimerEvent( void *p_context )
{
    ...
    else
    {
		...
		MacStateCheckTimerTime = MAC_STATE_CHECK_TIMEOUT;
        //TimerStart( &MacStateCheckTimer );
		app_timer_start(MacStateCheckTimer, APP_TIMER_TICKS(MacStateCheckTimerTime, APP_TIMER_PRESCALER), NULL);
...
static LoRaMacStatus_t ScheduleTx( void )
{
    ...
    else
    {
		...
		TxDelayedTimerTime = dutyCycleTimeOff;
        //TimerStart( &TxDelayedTimer );
		app_timer_start(TxDelayedTimer, APP_TIMER_TICKS(TxDelayedTimerTime, APP_TIMER_PRESCALER), NULL);
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
	...
	app_timer_create(&MacStateCheckTimer, APP_TIMER_MODE_REPEATED, OnMacStateCheckTimerEvent);

    //TimerSetValue( &MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT );
	MacStateCheckTimerTime = MAC_STATE_CHECK_TIMEOUT;
    //TimerInit( &TxDelayedTimer, OnTxDelayedTimerEvent );
	app_timer_create(&TxDelayedTimer, APP_TIMER_MODE_REPEATED, OnTxDelayedTimerEvent);
    //TimerInit( &RxWindowTimer1, OnRxWindow1TimerEvent );
	app_timer_create(&RxWindowTimer1, APP_TIMER_MODE_REPEATED, OnRxWindow1TimerEvent);
    //TimerInit( &RxWindowTimer2, OnRxWindow2TimerEvent );
	app_timer_create(&RxWindowTimer2, APP_TIMER_MODE_REPEATED, OnRxWindow2TimerEvent);
    //TimerInit( &AckTimeoutTimer, OnAckTimeoutTimerEvent );
	app_timer_create(&AckTimeoutTimer, APP_TIMER_MODE_REPEATED, OnAckTimeoutTimerEvent);
