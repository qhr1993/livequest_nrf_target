 ...
 */
void SX1276OnDio0Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 1 IRQ callback
 */
void SX1276OnDio1Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 2 IRQ callback
 */
void SX1276OnDio2Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 3 IRQ callback
 */
void SX1276OnDio3Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 4 IRQ callback
 */
void SX1276OnDio4Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 5 IRQ callback
 */
void SX1276OnDio5Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );
 ...
 */
nrf_drv_gpiote_evt_handler_t DioIrq[] = { SX1276OnDio0Irq, SX1276OnDio1Irq,
...
APP_TIMER_DEF(TxTimeoutTimer);
APP_TIMER_DEF(RxTimeoutTimer);
APP_TIMER_DEF(RxTimeoutSyncWord);

uint32_t TxTimeoutTimerTime;
uint32_t RxTimeoutTimerTime;
uint32_t RxTimeoutSyncWordTime;
...
void SX1276Init( RadioEvents_t *events )
{
	...
	app_timer_create(&TxTimeoutTimer, APP_TIMER_MODE_REPEATED, SX1276OnTimeoutIrq);
	app_timer_create(&RxTimeoutTimer, APP_TIMER_MODE_REPEATED, SX1276OnTimeoutIrq);
	app_timer_create(&RxTimeoutSyncWord, APP_TIMER_MODE_REPEATED, SX1276OnTimeoutIrq);
...
void SX1276SetSleep( void )
{
	...
	app_timer_stop(RxTimeoutTimer);
	app_timer_stop(TxTimeoutTimer);
...
void SX1276SetRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
		...
		RxTimeoutTimerTime = timeout;
        //TimerStart( &RxTimeoutTimer );
		app_timer_start(RxTimeoutTimer, APP_TIMER_TICKS(RxTimeoutTimerTime, APP_TIMER_PRESCALER), NULL);
    ...
    if( SX1276.Settings.Modem == MODEM_FSK )
    {
        ...
        if( rxContinuous == false )
        {
			...
			RxTimeoutSyncWordTime = SX1276.Settings.Fsk.RxSingleTimeout;
            //TimerStart( &RxTimeoutSyncWord );
			app_timer_start(RxTimeoutSyncWord, APP_TIMER_TICKS(RxTimeoutSyncWordTime, APP_TIMER_PRESCALER), NULL);
        }
    }
    else
    {
...
void SX1276SetTx( uint32_t timeout )
{
	...
	app_timer_start(TxTimeoutTimer, APP_TIMER_TICKS(TxTimeoutTimerTime, APP_TIMER_PRESCALER), NULL);
...
void SX1276Reset( void )
{
    ...
    nrf_drv_gpiote_in_uninit(RADIO_RESET);
		nrf_drv_gpiote_out_config_t cfgTmpOut = GPIOTE_CONFIG_OUT_SIMPLE(0);
    SX1276.Reset_out = cfgTmpOut;
    nrf_drv_gpiote_out_init(RADIO_RESET,&(SX1276.Reset_out));

    // Wait 1 ms
    DelayMs( 1 );

    nrf_drv_gpiote_out_uninit(RADIO_RESET);

    // Configure RESET as input
    //GpioInit( &SX1276.Reset, RADIO_RESET, PIN_INPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
		nrf_drv_gpiote_in_config_t cfgTmpIn =  	GPIOTE_CONFIG_IN_SENSE_TOGGLE(false);
    SX1276.Reset_in = cfgTmpIn;
    nrf_drv_gpiote_in_init(RADIO_RESET,&(SX1276.Reset_in),NULL);
...
void SX1276WriteBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    uint8_t opcode = (addr | 0x80);
    nrf_drv_spi_transfer(&(SX1276.Spi_master), &opcode,1,NULL,0);
    nrf_drv_spi_transfer(&(SX1276.Spi_master), buffer,size,NULL,0);
...
void SX1276ReadBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    // //NSS = 1;
    // GpioWrite( &SX1276.Spi.Nss, 1 );
    uint8_t opcode = (addr | 0x7F);
    nrf_drv_spi_transfer(&(SX1276.Spi_master), &opcode,1,NULL,0);
    nrf_drv_spi_transfer(&(SX1276.Spi_master), NULL,0,buffer,size);
}
    ...
    case RF_RX_RUNNING:
        if( SX1276.Settings.Modem == MODEM_FSK )
        {
            ...
            else
            {
				...
				app_timer_stop(RxTimeoutSyncWord);
            }
        }
    ...
    switch( SX1276.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                if( !nrf_drv_gpiote_in_is_set(RADIO_DIO_4)  )
                {
